...
from get_lora_config import get_lora_config
...
def save_model(args, epoch, model, model_without_ddp, optimizer, loss_scaler, model_ema=None, optimizer_disc=None,
    ...
    selective_save = getattr(args, 'output_dir', False) and getattr(args, 'save_ckpt', False) and getattr(args,
                                                                                                          'Encoder_Only',
                                                                                                          False)

    if not getattr(args, 'enable_deepspeed', False):
        print('save Encoder only')
        ...
        for checkpoint_path in checkpoint_paths:
            ...
            if selective_save:
                ...
                filtered_state_dict = {k: v for k, v in full_state_dict.items() if 'patch_embed' in k}
                to_save = {
        ...
        print('save all parameter')
        client_state = {'epoch': epoch}
        if model_ema is not None:
            client_state['model_ema'] = get_state_dict(model_ema)
        if selective_save:
            # 仅保存名称包含 'patch_embed' 的参数
            full_state_dict = model_without_ddp.state_dict()
            filtered_state_dict = {k: v for k, v in full_state_dict.items() if 'patch_embed' in k}
            client_state['model'] = filtered_state_dict
        else:
            client_state['model'] = model_without_ddp.state_dict()
    ...
    if not getattr(args, 'enable_deepspeed', False):
        ...
        if args.use_lora:
            ...
            if args.stage_2:
                ...
                print('----------------------------lora1---------------------------')
                incompatible_keys = model_without_ddp.load_adapter(args.lora_1_path,adapter_name="default")
                print('Incompatible keys:', incompatible_keys)
                model_without_ddp.set_adapter("default")
                sd = model_without_ddp.state_dict()
                ...
                print('----------------------------lora2---------------------------')
                if args.eval:
                    model_without_ddp = PeftModel.from_pretrained(model_without_ddp, args.lora_2_path)
...
def merge_lora_into_base(model):
    ...
    for name, module in model.named_modules():
        if hasattr(module, "lora_A") and hasattr(module, "lora_B"):
            ...
            base_weight = getattr(module.base_layer, "weight")
            lora_A = getattr(module.lora_A, "default").weight
            lora_B = getattr(module.lora_B, "default").weight


            lora_weight = lora_B @ lora_A
            base_weight.data += lora_weight.data  


            delattr(module, "lora_A")
            delattr(module, "lora_B")

    print("LoRA merge successed！")
...
def freeze_except_temporalconv(model, encoder_flag):
    """
    Freeze all model parameters except those in TemporalConv layers.

    Args:
        model: The model containing TemporalConv.
        encoder_flag: Boolean flag (args.encoder) to determine if freezing is required.
    """
    if encoder_flag:
        for name, param in model.named_parameters():
            # Check if the parameter belongs to TemporalConv
            if "patch_embed" in name:
                param.requires_grad = True
            else:
                param.requires_grad = False
def freeze_temporalconv(model, encoder_flag):
    """
    Freeze all model parameters except those in TemporalConv layers.
