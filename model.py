...
class TemporalConv(nn.Module):
    def __init__(self, in_chans=1, out_chans=8):
        ...
        # Frequency-domain feature extraction layers
        self.freq_conv1 = nn.Conv2d(1, out_chans, kernel_size=(1, 3), padding=(0, 1))
        self.freq_gelu1 = nn.GELU()
        self.freq_norm1 = nn.GroupNorm(4, out_chans)
        self.freq_conv4 = nn.Conv2d(out_chans, out_chans, kernel_size=(1, 15), stride=(1, 8), padding=(0, 7))
        self.freq_gelu4 = nn.GELU()
        self.freq_norm4 = nn.GroupNorm(4, out_chans)
        self.freq_conv2 = nn.Conv2d(out_chans, out_chans, kernel_size=(1, 3), padding=(0, 1))
        self.freq_gelu2 = nn.GELU()
        self.freq_norm2 = nn.GroupNorm(4, out_chans)
        self.freq_conv3 = nn.Conv2d(out_chans, out_chans, kernel_size=(1, 3), padding=(0, 1))
        self.freq_gelu3 = nn.GELU()
        self.freq_norm3 = nn.GroupNorm(4, out_chans)

        # Concatenation and mapping layer
        self.fc = nn.Linear(out_chans * 2, out_chans)  # Concatenate two out_chans, map back to out_chans


    def forward(self, x):
        # Time-domain feature extraction
        x_times = rearrange(x, 'B N A T -> B (N A) T')
        x_times = x_times.unsqueeze(1)  # (B, 1, N*A, T)
        x_times = self.gelu1(self.norm1(self.conv1(x_times)))
        x_times = self.gelu2(self.norm2(self.conv2(x_times)))
        x_times = self.gelu3(self.norm3(self.conv3(x_times)))
        x_times = rearrange(x_times, 'B C NA T -> B NA T C')  # (B, N*A, T, C)

        # Frequency-domain feature extraction
        x_freq = torch.fft.fft(x, dim=-1)
        x_freq = torch.abs(x_freq)
        x_freq = rearrange(x_freq, 'B N A T -> B (N A) T')
        x_freq = x_freq.unsqueeze(1)  # (B, 1, N*A, T)
        x_freq = self.freq_gelu1(self.freq_norm1(self.freq_conv1(x_freq)))
        x_freq = self.freq_gelu4(self.freq_norm4(self.freq_conv4(x_freq)))
        x_freq = self.freq_gelu2(self.freq_norm2(self.freq_conv2(x_freq)))
        x_freq = self.freq_gelu3(self.freq_norm3(self.freq_conv3(x_freq)))
        x_freq = rearrange(x_freq, 'B C NA T -> B NA T C')  # (B, N*A, T, C)

        # Concatenate time and frequency features
        x_concat = torch.cat([x_times, x_freq], dim=-1)  # (B, N*A, T, 2C)

        # Linear mapping to original dimension
        x_output = self.fc(x_concat)  # (B, N*A, T, C)

        # Final reshape to (B, N*A, T*C)
        x_output = rearrange(x_output, 'B NA T C -> B NA (T C)  ')
...
class NeuralTransformer(nn.Module):
    ...
    def forward_features(self, x, input_chans=None, return_patch_tokens=False, return_all_tokens=False, **kwargs):
        batch_size, n, a, t = x.shape #torch.Size([64, 19, 10, 200])
        ...
        if self.time_embed is not None:
            ...
            #time_embed[:, 0:input_time_window, :]为切片，unsqueeze(1)为添加一个新维度，expand(batch_size, nc, -1, -1)第0和第一维度扩展到batch_size, nc，其他的不变，flatten(1, 2)从第一到第二维度展平到一个维度
            x[:, 1:, :] += time_embed
