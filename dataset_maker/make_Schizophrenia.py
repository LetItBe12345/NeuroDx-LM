...
chOrder_standard = [ 'FP2', 'F8', 'T4', 'T6', 'O2', 'FP1', 'F7', 'T3', 'T5', 'O1', 'F4', 'C4', 'P4', 'F3', 'C3', 'P3', 'FZ', 'CZ', 'PZ']
    ...
    for file in os.listdir(fetch_folder):
        if sub in file:
            ...
            try:
                ...
                if "Fp1" in ch_names:
                    raw.rename_channels({"Fp1": "FP1"})
                if "Fz" in ch_names:
                    raw.rename_channels({"Fz": "FZ"})
                if "Cz" in ch_names:
                    raw.rename_channels({"Cz": "CZ"})
                if "Pz" in ch_names:
                    raw.rename_channels({"Pz": "PZ"})
                ...
                if len(raw.ch_names) == len(chOrder_standard):
                    ...
                    raise Exception(
                        f"Channel count mismatch. Found {len(raw.ch_names)}, expected {len(chOrder_standard)}")
