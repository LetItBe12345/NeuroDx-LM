...
from engine_for_model import train_one_epoch, evaluate
from utils import NativeScalerWithGradNormCount as NativeScaler
import utils
from scipy import interpolate
import model
from create_lora import create_lora
...
def get_args():
    ...
    parser.add_argument('--dataset', default='TUAB', type=str,
                        help='dataset: TUAB | TUEV |CHB-MIT|Schizophrenia ')
    parser.add_argument('--use_lora', action='store_true', default=False)
    parser.add_argument('--lora_rank', default=8, type=int, help='Rank of LoRA')
    parser.add_argument('--lora_alpha', default=32, type=int, help='Scaling factor for LoRA')
    parser.add_argument('--lora_dropout', default=0.1, type=float, help='Dropout rate for LoRA layers')
    parser.add_argument('--lora_bias', default="none", type=str, help='whether lora bias or not')
    parser.add_argument('--stage_3', default=False, action='store_true')
    parser.add_argument('--lora_1_path', default='', help='load lora weights from stage 2')
    parser.add_argument('--lora_2_path', default='', help='load lora weights from stage 3')
    parser.add_argument('--stage_2', default=False, action='store_true')
    parser.add_argument('--output_dir_best_model', default='',
                        help='path where to save, empty for no saving')
    parser.add_argument('--Encoder_Freeze', default=True, action='store_true')
    ...
    parser.add_argument('--early_stopping_metric', default='accuracy', type=str,
                        help='Metric to monitor for early stopping (accuracy, loss, etc.)')
...
def get_dataset(args):
    if args.dataset == 'TUAB':
        train_dataset, test_dataset, val_dataset = utils.prepare_TUAB_dataset( "/home/jin/LaBraM/Datasets/TUH_Abnormal/v3.0.0/edf/processed/")
    ...
    elif args.dataset == 'CHB':
        train_dataset, test_dataset, val_dataset = utils.prepare_TUAB_dataset("/home/jin/LaBraM/Datasets/chb-mit-scalp-eeg-database-1.0.0/processed/")
        ch_names = [ "FP1-F7", "F7-T7", "T7-P7", "P7-O1", "FP2-F8", "F8-T8", "T8-P8" , "P8-O2", "FP1-F3", "F3-C3", "C3-P3", "P3-O1",
    "FP2-F4", "F4-C4", "C4-P4", "P4-O2"]
        args.nb_classes = 1
        metrics = ["pr_auc", "roc_auc", "accuracy", "balanced_accuracy"]
    elif args.dataset == 'Schizophrenia':
        train_dataset, test_dataset, val_dataset = utils.prepare_TUAB_dataset("/home/jin/LaBraM/Datasets/dataverse_files/processed/")
...
def main(args, ds_init):
    ...
    if args.finetune:
        ...
        if args.use_lora:
            model = create_lora(model,args)
        if args.stage_3:
            incompatible_keys = model.load_adapter(args.lora_1_path, adapter_name="default")
            for name, param in model.named_parameters():
                print(f"{name}: requires_grad = {param.requires_grad}")
            sd = model.state_dict()
            print(sd["base_model.model.blocks.11.attn.proj.lora_B.default.weight"])
    ...
    if args.Encoder_Only:
        utils.freeze_except_temporalconv(model, args.Encoder_Only)
        print('we have freeze every parameter except temporal conv layer')
    if args.Encoder_Freeze:
        utils.freeze_temporalconv(model, args.Encoder_Freeze)
        print('we have freeze every parameter about temporal conv layer')
    ...
    for epoch in range(args.start_epoch, args.epochs):
        if args.output_dir and args.save_ckpt and args.use_lora:
            raise ValueError("output_dir, save_ckpt, and use_lora cannot all be set simultaneously.")
        if args.distributed:
        ...
        if args.output_dir and args.use_lora:
            state_dict = get_peft_state_maybe_zero_3(model_without_ddp.named_parameters())
            model.module.save_pretrained(args.output_dir, state_dict=state_dict)
        ...
        if data_loader_val is not None:
            ...
            current_metric = val_stats[args.early_stopping_metric]
            if current_metric > best_metric + args.min_delta:
                best_metric = current_metric
                patience_counter = 0
                best_epoch = epoch
                # Save best model based on configuration
                if args.output_dir and args.use_lora:
                    state_dict = get_peft_state_maybe_zero_3(model_without_ddp.named_parameters())
                    if args.output_dir_best_model:
                        save_path = args.output_dir_best_model
                    else:
                        save_path = os.path.join(args.output_dir, "best_model")
                    model.module.save_pretrained(save_path, state_dict=state_dict)
                elif args.output_dir and args.save_ckpt:
                    utils.save_model(
                        args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                ...
                if patience_counter >= args.patience:
                    print(f'Early stopping triggered! Best {args.early_stopping_metric}: {best_metric:.4f} at epoch {best_epoch}')
                    break
